...
enum SoundButtonClickHeld {
    SOUND_BUTTON_RELEASED,
...
void check_sound_mode_menu_clicked_buttons(struct Object *soundModeButton) {
    s32 newPitch;
    s32 newTempo;
    u8 buttonPressedOrHeld = SOUND_BUTTON_RELEASED;
    ...
    if (soundModeButton->oMenuButtonState == MENU_BUTTON_STATE_FULLSCREEN) {
        ...
        for (buttonID = MENU_BUTTON_OPTION_MIN; buttonID < MENU_BUTTON_OPTION_MAX; buttonID++) {
            ...
            if (check_clicked_button(buttonX, buttonY, 22.0f) == TRUE) {
                buttonPressedOrHeld = SOUND_BUTTON_PRESSED;
            ...
            if (check_held_button(buttonX, buttonY, 22.0f) == TRUE) {
                buttonPressedOrHeld = SOUND_BUTTON_HELD;
                if (buttonID == MENU_BUTTON_PITCH_UP || buttonID == MENU_BUTTON_PITCH_DOWN || buttonID == MENU_BUTTON_TEMPO_UP || buttonID == MENU_BUTTON_TEMPO_DOWN) {
                    ...
                    if (buttonID == MENU_BUTTON_PITCH_UP || buttonID == MENU_BUTTON_PITCH_DOWN) {
                        newPitch = (s32) (gPitchModifier * 100.0f + 0.5f);
                        ...
                        gPitchModifier = (f32) newPitch / 100.0f;
                    } else {
                        newTempo = (s32) (gTempoModifier * 100.0f + 0.5f);
                        ...
                        gTempoModifier = (f32) newTempo / 100.0f;
                    }
                }
            ...
            if (buttonPressedOrHeld != SOUND_BUTTON_RELEASED) {
                break;
            }
        }
    }
    if (buttonPressedOrHeld != SOUND_BUTTON_HELD) {
...
void handle_cursor_button_input(void) {
    ...
    if (sSelectedButtonID == MENU_BUTTON_SCORE_FILE_A || sSelectedButtonID == MENU_BUTTON_SCORE_FILE_B
        ...
        if (gPlayer3Controller->buttonPressed
            ...
            & (A_BUTTON | B_BUTTON | START_BUTTON))
#endif
            && (sHeldPos[0] != -10000 && sHeldPos[1] != -10000)
        ) {
            sHeldPos[0] = sCursorPos[0];
            sHeldPos[1] = sCursorPos[1];
        }
...
void update_pitch_tempo_strings() {
    u32 pitch = (u32) (gPitchModifier * 100.0f + 0.5f);
    u32 tempo = (u32) (gTempoModifier * 100.0f + 0.5f);
...
void print_up_down_arrows() {
    ...
    if ((s32) (gPitchModifier * 100.0f + 0.5f) == 400) {
    ...
    if ((s32) (gPitchModifier * 100.0f + 0.5f) == 0) {
        gDPSetEnvColor(gDisplayListHead++, 127, 127, 127, sTextBaseAlpha * 0.5f);
    }
    else {
        gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    ...
    if ((s32) (gTempoModifier * 100.0f + 0.5f) == 400) {
        gDPSetEnvColor(gDisplayListHead++, 127, 127, 127, sTextBaseAlpha * 0.5f);
    }
    else {
        gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    ...
    if ((s32) (gTempoModifier * 100.0f + 0.5f) == 0) {
        gDPSetEnvColor(gDisplayListHead++, 127, 127, 127, sTextBaseAlpha * 0.5f);
    }
    else {
        gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
...
Gfx *geo_file_select_strings_and_menu_cursor(s32 callContext, UNUSED struct GraphNode *node, UNUSED Mat4 mtx) {
    ...
    if (sAudioSwapTimer >= 0 && sAudioSwapTimer < 10) {
        ...
        if (sAudioSwapTimer == 10 || instAudioSwap) {
            if (seqNum > 0) {
#if defined(BETTER_REVERB) && defined(BETTER_REVERB_SOUND_PLAYER_PRESET)
                gBetterReverbPresetValue = BETTER_REVERB_SOUND_PLAYER_PRESET;
#endif
